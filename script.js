...
class Currency {
    constructor(CZK,EUR,GBP,USD,CHF) {
        this.CZK = Number(CZK.toPrecision(3));
        this.EUR = Number(EUR.toPrecision(3));
        this.GBP = Number(GBP.toPrecision(3));
        this.USD = Number(USD.toPrecision(3));
        this.CHF = Number(CHF.toPrecision(3));
    }
};
...
function fetchAndSaveCurrenciesRates(baseCurrency, currencyObj,currencyToCzkObj) {
    fetch(`https://api.exchangerate.host/latest?base=${baseCurrency}`)
    .then(response => response.json())
    .then(data => {
        let rates = data.rates

        currency = new currencyObj(rates.CZK,rates.EUR,rates.GBP,rates.USD,rates.CHF);
        currencyToCZK = new currencyToCzkObj(rates.CZK,rates.EUR,rates.GBP,rates.USD,rates.CHF);
            }
...
resultNum = (((currency[currencyOne]) * (currency[currencyTwo])) * parseFloat(numberOfMoney));
} else {
    resultNum = ((currency[currencyTwo] / currency[currencyOne]) * parseFloat(numberOfMoney));
}
    input.value = resultNum.toFixed(2);
};
...
function FormatDateAndGetApiInfo (date) {
    ...
    dateOfListInput.value = formatTodayStr
        ...
        .then(response => response.json())
        .then(data => {currencyToCZK = new CurrencyToCZK(data.rates.CZK,data.rates.EUR,data.rates.GBP,data.rates.USD,data.rates.CHF);
...
function AppOn() {
    ...
    fetchAndSaveCurrenciesRates("CZK", Currency, CurrencyToCZK);
